...
class TierValidation(models.AbstractModel):
    ...
    def _tier_validation_get_current_state_value(self):
        ...
        if self._tier_validation_state_field_is_computed and isinstance(self.id, int):
            # SQL.identifier rather than psycopg2's AsIs: it is Odoo's own
            # documented way to interpolate an identifier, it asserts the name
            # IS an identifier before quoting it, and it keeps the module free
            # of a direct psycopg2 import.
            self.env.cr.execute(
                SQL(
                    "select %(field)s from %(table)s where id = %(res_id)s",
                    field=SQL.identifier(self._state_field),
